...
class DatasetAnalyzer:
    ...
    def detect_potential_issues(df: pd.DataFrame) -> Dict[str, Any]:
        ...
        issues = {
            ...
            "potential_noise": int(
                (df["crimeaditionalinfo"].str.contains(r"[^\w\s.,!?@#$%&*()]")).sum()
            ),
        }
        return issues
    ...
    def prepare_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Preprocess text
            prepared_df["processed_text"] = prepared_df["crimeaditionalinfo"].apply(
            ...
            for column in ["category", "sub_category"]:
                ...
                valid_classes = class_counts[
                    class_counts >= self.min_samples_per_class
                ].index

                if len(valid_classes) == 0:
                    raise ValueError(
                        f"No classes have the minimum required {self.min_samples_per_class} samples"
                    )
                ...
                # Encode labels
                prepared_df[f"{column}_encoded"] = self.label_encoders[
                    column
                ].transform(prepared_df[column])

                # Log removed classes
                removed_classes = set(class_counts.index) - set(valid_classes)
                if removed_classes:
                    logging.warning(
                        f"Removed {len(removed_classes)} {column} classes with fewer than "
...
def clean_dataframe(
    ...
    cleaning_steps = [
        ...
        (
            "Remove empty texts",
            lambda x: x[x["crimeaditionalinfo"].str.strip() != ""],
            "Removed empty texts",
        ),
        (
            "Fill missing values",
        ...
        (
            "Convert to string",
            lambda x: x.assign(crimeaditionalinfo=x["crimeaditionalinfo"].astype(str)),
            "Converted text to string",
        ),
        (
            "Clean special characters",
            lambda x: x.assign(
                crimeaditionalinfo=x["crimeaditionalinfo"].str.replace(
                    r"[^\w\s.,!?@#$%&*()]", " ", regex=True
                )
            ),
    ...
    if is_training:
        cleaning_steps.extend(
            [
                (
                    "Remove duplicates",
                    lambda x: x.drop_duplicates(subset="crimeaditionalinfo"),
                    "Removed duplicate texts",
                ),
                (
                    "Remove very short texts",
                    lambda x: x[x["crimeaditionalinfo"].str.len() >= 20],
...
def train_and_save_model(
    ...
    try:
        ...
        for column in ["category", "sub_category"]:
            ...
            logger.info(
                f"Classes with 2-4 samples: {sum((class_counts >= 2) & (class_counts < 5))}"
            )
            logger.info(f"Classes with 5+ samples: {sum(class_counts >= 5)}")

        # Clean data
        train_df_cleaned, train_cleaning_report = clean_dataframe(
            train_df, is_training=True
        )
        test_df_cleaned, test_cleaning_report = clean_dataframe(
        ...
        logger.info(
            f"Preparing data with minimum {min_samples_per_class} samples per class..."
        )
        try:
            ...
            logger.info(
                f"Training data shape after filtering: {train_df_filtered.shape}"
            )
            logger.info(f"Test data shape after filtering: {test_df_filtered.shape}")

            # Create validation set
            train_final, val_df = train_test_split(
                train_df_filtered, test_size=0.1, random_state=42
            )
            ...
            metrics = {
                ...
                "class_counts": {
                    ...
                    "sub_category": train_df_filtered["sub_category"]
                    .value_counts()
                    .to_dict(),
                },
            }
        ...
        with open(metrics_file, "r") as f:
            ...
            if overlap["category"]["test_only"] or overlap["sub_category"]["test_only"]:
                ...
                if overlap["sub_category"]["test_only"]:
                    print(
                        f"Sub-categories: {', '.join(overlap['sub_category']['test_only'])}"
                    )
    except Exception as e:
